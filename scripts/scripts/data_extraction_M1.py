...
def extract_answers_sequence(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Detect answer lines: they start with '['
            if stripped.startswith("["):
                ...
                # Once we have 4 answer lines, determine the selected option
                if len(current_question_answers) == 4:
                    if 1 in current_question_answers:
                        # The index of the '1' corresponds to the chosen option
                        answers.append(current_question_answers.index(1) + 1)
                    else:
                        answers.append(0)  # unanswered
                    current_question_answers = []
